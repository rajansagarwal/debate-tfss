...
export default function crashCourse() {
    ...
    const debateSlides = {'SlideName': [current_date, 'SlideInfo', 'SlideLink'], 'SlideName2': [current_date, 'SlideInfo2', 'SlideLink']}
    const lessonSlides = Object.entries(debateSlides).map( ([slideTitle, slideData]) => 
    <div className={styles.slideBox}>
        <h1>{slideTitle}</h1>
        <h2>{slideData[0]}</h2>
        <p>{slideData[1]}</p>
        <p> <b>Link: </b> <a href={slideData[2]}><u>Slideshow</u></a></p>
    </div>
    )
    ...
    // Found below is sample static input
    const debateTopics = {"Why Jaival is Supreme": ["TopicSummary", "LinkToRoom"], "Why Jaival is the Best": ["TopicSummary", "LinkToRoom"], "Why Jaival is the best MMA Fighter at Turner":["TopicSummary", "LinkToRoom"]}
    const sampleTopics = Object.entries(debateTopics).map( ([debateTopic, debateRoomLst]) => 
    <div className={styles.topicBox}>
        <h1>{debateTopic}</h1>
        <h2>{debateRoomLst[0]}</h2>
        <p><b>Link to Debate Room: </b> <a href={debateRoomLst[1]}><u>Room</u></a></p>
    </div>
    )
